...
url_iotivity = "git://github.com/tizenteam/iotivity.git"
#branch_iotivity = "sandbox/pcoval/on/latest/mine"
SRCREV = "${branch_iotivity}"
#SRC_URI = "${url_iotivity};destsuffix=${S};branch=${branch_iotivity};protocol=http;"
SRC_URI = "git://github.com/iotivity/iotivity.git;branch=1.2-rel;protocol=http"
...
#   EXTRA_OESCONS += " SECURED=1"
    EXTRA_OESCONS += " WITH_CLOUD=0 "
    EXTRA_OESCONS += " WITH_PROXY=1 "
...
do_install() {
    ...
    #Workaround broken pkconfig
    make_dir ${D}${libdir}/pkgconfig/
    sed -e 's|^prefix=.*|prefix=/usr|g' -i ${S}/iotivity.pc
    copy_file ${S}/iotivity.pc ${D}${libdir}/pkgconfig/
    ln -fs ${D}${includedir}/iotivity/resource ${D}${includedir}/resource
    ln -fs ${D}${includedir}/iotivity/service ${D}${includedir}/service
    ln -fs ${D}${includedir}/iotivity/resource/*/*.h*  ${D}${includedir}/resource/
    ln -fs ${D}${includedir}/iotivity/resource/oc_logger/targets ${D}${includedir}/resource/
    ...
    #Resource directory
    if ${@bb.utils.contains('EXTRA_OESCONS', 'RD=TODO', 'true', 'false', d)}; then
